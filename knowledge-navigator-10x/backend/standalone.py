"""
Knowledge Navigator 10X - AI-Powered Backend
=============================================
Powered by Google Gemini 2.5 Flash (google-genai SDK).
Falls back to rich local KB responses if API unavailable.
"""
...
if GOOGLE_API_KEY:
    ...
    logger.warning("⚠️  No GOOGLE_API_KEY set. Using local KB responses.")
...
def extract_snippets(question: str, domain: str, max_snippets: int = 4) -> list[dict]:
    snippets = []
...
async def generate_with_gemini(question: str, persona: str, domain: str, context: str) -> str:
    """Call Gemini 2.5 Flash for a real AI response."""
...
DOMAIN_ACTIONS = {
    "audit":      ["Maintain a live controls inventory with test dates and owners", "Prepare evidence packs before any audit begins", "Conduct quarterly self-assessments", "Track all findings with RAG status and target dates", "Train control owners on audit interview techniques", "Document root cause analysis for every finding"],
    "risk":       ["Complete RCSA updates quarterly or upon material change", "Register all risks within 5 business days", "Assign named Risk Owners with accountability", "Set up KRI monitoring dashboards with alerts", "Escalate High/Critical risks to CRO within 24 hours", "Review risk register at monthly team meetings"],
    "compliance": ["Complete mandatory compliance training by December 31", "Maintain a compliance obligations register", "Report potential breaches to Compliance immediately", "Implement pre-trade compliance checks", "Respond to GDPR requests within 30 days", "Conduct regular compliance self-assessments"],
    "controls":   ["Maintain a controls inventory with testing schedules", "Test all key controls at least annually", "Document testing workpapers for auditor review", "Remediate Significant Deficiencies within 90 days", "Implement Continuous Control Monitoring", "Escalate Material Weaknesses immediately"],
    "governance": ["Complete annual policy certification before year-end", "Review PolicyNet quarterly for updates", "Follow the formal exception process", "Ensure procedures align to Group Policies", "Participate in governance committees", "Report policy breaches formally"],
    "euda":       ["Register all Tier 1 and Tier 2 EUDAs within 30 days", "Assign a named Business Owner to each EUDA", "Document purpose, users, inputs, controls", "Store EUDAs in controlled locations", "Conduct annual risk assessment and recertification", "Plan migration of Tier 1 EUDAs to IT systems"],
    "automation": ["Register all automations before go-live", "Never grant bots elevated privileges", "Document and test the fallback manual process", "Set up automated failure alerts within 15 minutes", "Conduct quarterly automation performance reviews", "Decommission unused automations promptly"],
    "learning":   ["Complete mandatory training within 30 days of joining", "Discuss your development plan within 2 weeks", "Set 3–5 learning goals at year start", "Explore DB's certification sponsorship programme", "Attend monthly Learning Hour sessions", "Mentor junior colleagues to reinforce your own learning"],
    "hr":         ["Set annual goals in HR system by February 28", "Schedule mid-year review in June", "Discuss career development at least twice per year", "Familiarise yourself with myBenefits portal", "Use the EAP for personal support", "Complete all mandatory training"],
    "operations": ["Respond to P1 alerts within 15 minutes", "Document root cause in ServiceNow for all P1/P2", "Conduct daily service health checks", "Ensure runbooks are up-to-date and accessible", "Monitor SLA performance weekly", "Test BCP plans quarterly"],
}
...
def build_local_response(question: str, persona: str, domain: str, snippets: list[dict]) -> str:
    intro = PERSONA_INTRO.get(persona, "")
    domain_display = domain.replace("_", " ").title()

    # Build answer from KB snippets if available
    if snippets:
        kb_content = "\n\n".join(
            f"> *[{s['domain'].upper()}]* {s['content'][:400]}"
            for s in snippets[:3]
        )
        return f"""{intro}**📋 Answer**
Here is the relevant information from Deutsche Bank's {domain_display} knowledge base regarding your question: *"{question}"*
...
async def generate_top10_with_gemini(topic: str, domain: str, persona: str) -> str:
    if not gemini_client:
        return None
...
async def startup():
    load_knowledge_base()
    mode = f"Gemini {GEMINI_MODEL}" if gemini_client else "Local KB (no API key)"
    logger.info(f"🚀 Knowledge Navigator 10X ready | Mode: {mode} | http://localhost:8000")
...
if __name__ == "__main__":
    import uvicorn
    uvicorn.run("standalone:app", host="0.0.0.0", port=8000, reload=True)
